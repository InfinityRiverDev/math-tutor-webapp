...
export default function AdminPage({ user, subscription, reloadSubscription }) {
  ...
  if (page === "services")  return <Services goBack={() => setPage("admin")} user={user} goToChat={goToChat} />
  ...
  return <AdminHome user={user} setPage={setPage} isManager={user.id === MANAGER_ID} />
}
...
function AdminHome({ user, setPage, isManager }) {
  ...
  if (isManager) tabs.push({ id: "orders", label: "🎞️ Заказы" })
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.tabContent}>
        ...
        {activeTab === "orders"    && <OrdersTab user={user} setPage={setPage} />}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={tab.section}>
      {loading ? <Spinner /> : stats && !stats.error ? (
        ...
        <div style={tab.empty}>❌ Не удалось загрузить. Проверь admin_routes.py</div>
      ) : null}
  ...
  return (
    <div style={tab.section}>
      ...
      <textarea value={text} onChange={e => setText(e.target.value)}
        placeholder="Введите текст сообщения..." rows={6} style={tab.textarea} />
      ...
      {result && (
        <div style={{ ...tab.msg, color: result.error ? "#ef4444" : "#10b981" }}>
          {result.error ? `❌ ${result.error}` : `✅ Отправлено: ${result.sent} · Ошибки: ${result.failed}`}
        </div>
      )}
    </div>
  )
...
// ── Orders (для менеджера презентаций) ────────────────────────

function OrdersTab({ user, setPage }) {
  ...
  return (
    <div style={tab.section}>
      ...
      {loading ? <Spinner /> : chats.length === 0
        ...
        : chats.map(chat => (
          <button key={chat.user_id} style={{ ...tab.itemCard, cursor: "pointer", width: "100%", textAlign: "left" }}
            onClick={() => setSelectedChat(chat)}>
            <div style={tab.itemMain}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={tab.itemName}>{chat.name || `User ${chat.user_id}`}</span>
                {chat.unread > 0 && (
                  <span style={orders.badge}>{chat.unread}</span>
                )}
              </div>
              <span style={tab.itemSub}>
                {chat.last_msg
                  ? chat.last_msg.length > 40 ? chat.last_msg.slice(0, 40) + "…" : chat.last_msg
                  : "Нет сообщений"}
...
// ── ManagerChat — чат менеджера с конкретным юзером ───────────
...
function ManagerChat({ user, targetUserId, targetName, goBack }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      formData.append("to_id", targetUserId)
      const res = await fetch(`${API}/billing/chat/send-file`, {
        method: "POST",
        body: formData
      })
      const d = await res.json()
      ...
      <div style={mc.chat} ref={chatRef}>
        {messages.length === 0 && (
          <div style={mc.empty}>
            ...
            <div style={mc.emptyD}>Здесь появятся сообщения</div>
          </div>
        )}
        {messages.map(msg => {
              ...
              <div style={isMe ? mc.bubbleMe : mc.bubbleThem}>
                ...
                {msg.file_id && (
                  <div style={mc.fileMsg}>
                    📎 <span style={mc.fileName}>{msg.file_name || "Презентация.pptx"}</span>
...
// Нужен React для useRef в ManagerChat
