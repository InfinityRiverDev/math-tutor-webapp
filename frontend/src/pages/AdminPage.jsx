...
import { API, ADMIN_IDS, MANAGER_ID, PRINT_MANAGER_ID } from "../App"
...
import OrderChat from "./OrderChat"   // ← правильный регистр!

// Менеджеры с вкладкой «Заказы»
const MANAGER_IDS = [MANAGER_ID, PRINT_MANAGER_ID]
...
function AdminHome({ user, setPage, isManager }) {
  ...
  // Вкладка «Заказы» только для менеджеров
...
function PlansTab() {
  ...
  const save = async () => {
    if (!form.name || !form.price || !form.duration_days) {
      showMsg("Заполни все поля", false)
      return
    }
    setLoading(true)
    try {
      ...
      const d      = await res.json()
      if (d.success || d.plan_id) {
        showMsg(view === "create" ? "✅ Тариф создан!" : "✅ Обновлён!")
        setView("list")
        load()
      } else {
  ...
  const deletePlan = async (id) => {
    ...
    try {
      ...
      showMsg("🗑 Удалено")
      load()
    } catch { showMsg("Ошибка удаления", false) }
  }
  ...
  if (view !== "list") return (
    <div style={tab.section}>
      ...
      <div style={tab.btnRow}>
        ...
        <button style={tab.btnPrimary} onClick={save} disabled={loading}>
          {loading ? "..." : "Сохранить"}
        </button>
      </div>
      ...
      {!creating ? (
        ...
        <div style={tab.formCard}>
          ...
          <div style={tab.btnRow}>
            ...
            <button style={tab.btnPrimary} onClick={create} disabled={loading}>
              {loading ? "..." : "Создать"}
            </button>
          </div>
        </div>
...
function LecturesTab() {
  ...
  const deleteSubject = async (id) => {
    if (!confirm("Удалить предмет и все его лекции?")) return
    try {
  ...
  if (selectedSubj) return (
    <div style={tab.section}>
      ...
      <div style={tab.hint}>📌 Загрузка PDF — через бота командой /admin</div>
...
function BroadcastTab() {
  ...
  return (
    <div style={tab.section}>
      ...
      <div style={tab.hint}>Поддерживается HTML: &lt;b&gt;жирный&lt;/b&gt;, &lt;i&gt;курсив&lt;/i&gt;</div>
      <textarea
        value={text}
      ...
      />
      <button
        style={{ ...tab.btnPrimary, opacity: loading || !text.trim() ? 0.5 : 1 }}
...
// ── Orders — чаты с заказчиками (только для менеджеров) ───────────
...
function OrdersTab({ user }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await fetch(`${API}/billing/chat/orders?manager_id=${user.id}`)
      const d   = await res.json()
      const chatsWithNames = await Promise.all((d.chats ?? []).map(async chat => {
        try {
          const r    = await fetch(`${API}/billing/chat/user-info?user_id=${chat.user_id}`)
          const info = await r.json()
          return { ...chat, name: info.name, username: info.username, group: info.group }
        } catch {
          return { ...chat, name: `User ${chat.user_id}` }
        }
      }))
  ...
  if (selectedChat) return (
    <ManagerChat
      ...
      goBack={() => { setSelectedChat(null); load() }}
    />
  )
  ...
  return (
    <div style={tab.section}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={tab.sectionLabel}>Заказы</div>
        <button style={tab.btn} onClick={load}>🔄</button>
      </div>
      {loading ? <Spinner /> : chats.length === 0
        ? <div style={tab.empty}>📭 Заказов пока нет</div>
        : chats.map(chat => (
          <button
            key={chat.user_id}
            style={{ ...tab.itemCard, cursor: "pointer", width: "100%", textAlign: "left" }}
            onClick={() => setSelectedChat(chat)}
          >
            <div style={tab.itemMain}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={tab.itemName}>{chat.name || `User ${chat.user_id}`}</span>
                {chat.unread > 0 && <span style={oStyle.badge}>{chat.unread}</span>}
              </div>
              <span style={tab.itemSub}>
                {chat.last_msg
                  ? (chat.last_msg.length > 42 ? chat.last_msg.slice(0, 42) + "…" : chat.last_msg)
                  : "Нет сообщений"}
                {chat.group ? ` · Группа ${chat.group}` : ""}
              </span>
            </div>
...
// ── ManagerChat — полноэкранный чат с заказчиком ──────────────────
...
function ManagerChat({ user, targetUserId, targetName, goBack }) {
  ...
  const loadMessages = async () => {
    try {
      const r = await fetch(`${API}/billing/chat/messages?user_a=${user.id}&user_b=${targetUserId}`)
  ...
  const send = async () => {
    ...
    try {
      await fetch(`${API}/billing/chat/send`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ from_id: user.id, to_id: targetUserId, text })
      })
      await loadMessages()
    } catch {}
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!file) return
    if (!file.name.endsWith(".pptx")) { alert("Только файлы .pptx"); return }
    setUploading(true)
    try {
      const formData = new FormData()
      formData.append("file", file)
      formData.append("from_id", String(user.id))
      formData.append("to_id",   String(targetUserId))
      const res = await fetch(`${API}/billing/chat/send-file`, { method: "POST", body: formData })
      const d   = await res.json()
      if (d.success) await loadMessages()
  ...
  return (
    <div style={mc.root}>
      ...
      <div style={mc.chat} ref={chatRef}>
        {messages.length === 0 && (
          <div style={mc.empty}>
            ...
            <div style={mc.emptyT}>Диалог пустой</div>
            <div style={mc.emptyD}>Начните переписку с заказчиком</div>
          </div>
        )}
        {messages.map(msg => {
          ...
          return (
            <div key={msg._id} style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start", marginBottom: 8 }}>
              ...
              <div style={isMe ? mc.bubbleMe : mc.bubbleThem}>
                ...
                {msg.file_id && (
                  <div style={mc.fileMsg}>
                    📎 <span style={mc.fileName}>{msg.file_name || "Файл.pptx"}</span>
                    <span style={mc.fileHint}>(скачать через бот)</span>
                  </div>
                )}
      ...
      <div style={mc.inputArea}>
        <input ref={fileRef} type="file" accept=".pptx" style={{ display: "none" }} onChange={handleFileUpload} />
        <button
          style={{ ...mc.attachBtn, opacity: uploading ? 0.5 : 1 }}
          onClick={() => fileRef.current?.click()}
          disabled={uploading}
          title="Отправить .pptx"
        >
          {uploading ? "⏳" : "📎"}
        </button>
        <textarea
        ...
        />
        <button
          style={{ ...mc.sendBtn, opacity: !input.trim() || sending ? 0.4 : 1 }}
...
const s = {
  ...
  header: { position: "relative", overflow: "hidden", display: "flex", alignItems: "center", gap: 12, padding: "24px 20px 20px", background: "linear-gradient(160deg, #131929 0%, #0a0f1e 100%)", borderBottom: "1px solid rgba(255,255,255,0.05)" },
  headerGlow: { position: "absolute", top: -60, right: -40, width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,158,11,0.15) 0%, transparent 70%)", pointerEvents: "none" },
  adminBadge: { width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg, #f59e0b, #d97706)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, boxShadow: "0 0 0 3px rgba(245,158,11,0.25)" },
  headerInfo: { display: "flex", flexDirection: "column", gap: 2, flex: 1 },
  headerTitle: { fontSize: 18, fontWeight: 700, color: "#f1f5f9" },
  headerSub: { fontSize: 12, color: "rgba(255,255,255,0.35)" },
  adminTag: { background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.3)", color: "#f59e0b", fontSize: 10, fontWeight: 700, letterSpacing: "1px", padding: "4px 8px", borderRadius: 6 },
  ...
  userAccessBtn: { display: "flex", flexDirection: "column", alignItems: "center", gap: 4, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "10px 12px", cursor: "pointer", minWidth: 60 },
  userAccessLabel: { fontSize: 11, color: "rgba(255,255,255,0.5)", fontWeight: 600 },
  tabs: { display: "flex", overflowX: "auto", gap: 6, padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", scrollbarWidth: "none" },
  tab: { whiteSpace: "nowrap", padding: "8px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, color: "rgba(255,255,255,0.5)", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.15s" },
...
const tab = {
  ...
  statsCard: { position: "relative", overflow: "hidden", background: "linear-gradient(135deg, rgba(99,102,241,0.15), rgba(99,102,241,0.05))", border: "1px solid rgba(99,102,241,0.25)", borderRadius: 18, padding: "20px", display: "flex", flexDirection: "column", gap: 4 },
  statsGlow: { position: "absolute", top: -30, right: -30, width: 100, height: 100, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,102,241,0.3) 0%, transparent 70%)", pointerEvents: "none" },
  ...
  btn: { padding: "11px 16px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, color: "rgba(255,255,255,0.7)", fontSize: 14, fontWeight: 600, cursor: "pointer" },
  btnPrimary: { padding: "12px 16px", background: "linear-gradient(135deg, #6366f1, #4f46e5)", border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px rgba(99,102,241,0.3)" },
  btnRow: { display: "flex", gap: 8 },
  formCard: { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "14px", display: "flex", flexDirection: "column", gap: 10 },
  ...
  input: { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "10px 12px", color: "#f1f5f9", fontSize: 14, outline: "none", width: "100%", boxSizing: "border-box" },
  textarea: { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "12px 14px", color: "#f1f5f9", fontSize: 14, outline: "none", resize: "vertical", fontFamily: "inherit", lineHeight: 1.5, width: "100%", boxSizing: "border-box" },
  itemCard: { display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "12px 14px" },
  itemMain: { flex: 1, display: "flex", flexDirection: "column", gap: 3 },
  ...
  iconBtn: { background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 8, padding: "6px 8px", fontSize: 14, cursor: "pointer" },
  iconBtnDanger: { background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "6px 8px", fontSize: 14, cursor: "pointer" },
  ...
  empty: { textAlign: "center", color: "rgba(255,255,255,0.3)", padding: "24px 0", fontSize: 14 },
}

const oStyle = {
  badge: { background: "#ef4444", color: "#fff", borderRadius: "50%", width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 },
}

const mc = {
  root: { height: "100vh", background: "#0a0f1e", display: "flex", flexDirection: "column", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" },
  header: { display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "rgba(13,19,35,0.97)", borderBottom: "1px solid rgba(255,255,255,0.06)" },
  ...
  avatar: { width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 },
  hinfo: { flex: 1 },
  ...
  empty: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center", padding: "40px 24px" },
  emptyT: { fontSize: 17, fontWeight: 600, color: "#f1f5f9" },
  emptyD: { fontSize: 13, color: "rgba(255,255,255,0.35)", marginTop: 6 },
  msgAvatar: { width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg,#475569,#334155)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0, marginRight: 8, alignSelf: "flex-end", marginBottom: 2 },
  bubbleMe: { maxWidth: "78%", background: "linear-gradient(135deg,#6366f1,#4f46e5)", color: "#fff", borderRadius: "18px 18px 4px 18px", padding: "10px 14px", fontSize: 14.5, lineHeight: 1.55, boxShadow: "0 4px 20px rgba(99,102,241,0.3)" },
  bubbleThem: { maxWidth: "78%", background: "rgba(255,255,255,0.06)", color: "#e2e8f0", borderRadius: "18px 18px 18px 4px", border: "1px solid rgba(255,255,255,0.08)", padding: "10px 14px", fontSize: 14.5, lineHeight: 1.55 },
  ...
  inputArea: { background: "rgba(13,19,35,0.98)", padding: "10px 12px", display: "flex", alignItems: "flex-end", gap: 8, borderTop: "1px solid rgba(255,255,255,0.05)" },
  attachBtn: { width: 42, height: 42, borderRadius: "50%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 },
  textarea: { flex: 1, background: "rgba(255,255,255,0.06)", color: "#f1f5f9", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "10px 14px", minHeight: 40, maxHeight: 110, fontFamily: "inherit", fontSize: 14.5, resize: "none", outline: "none", lineHeight: 1.45, boxSizing: "border-box" },
  sendBtn: { width: 42, height: 42, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#4f46e5)", border: "none", color: "white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, boxShadow: "0 4px 16px rgba(99,102,241,0.45)", transition: "opacity 0.15s" },
}