...
export default function Tutor({ user, goBack }) {
  ...
  const send = async () => {
    ...
    const url = mode === "practice" ? `${API}/tutor/practice` : `${API}/tutor/`
  ...
  return (
    <div style={s.chatRoot}>
      ...
      <div style={s.chatHeader}>
        <button style={s.backBtn} onClick={() => setMode("menu")}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M11 14L6 9l5-5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      ...
      <div style={s.inputArea}>
        ...
        <button onClick={send} disabled={!input.trim() || loading} style={{ ...s.sendBtn, opacity: (!input.trim() || loading) ? 0.4 : 1 }}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M15.5 2.5L8 10M15.5 2.5L10.5 15.5L8 10M15.5 2.5L2.5 7L8 10" stroke="white" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      </div>
    </div>
...
const s = {
  ...
  typingDots: {
    display: "flex", gap: 5, padding: "2px 0", alignItems: "center",
  },
